use clang_bridge::{Index, hover, goto, completion};
use std::io::Write;
...
fn hover_markdown_returns_signature_and_brief() {
    let path = write_temp("cb_lsp_hover.hpp",
        "/// Adds two integers.\nint add(int a, int b);");
    let idx = Index::new();
...
fn goto_definition_resolves_function() {
    ...
    let path = write_temp("cb_lsp_goto.cpp",
...
fn completion_returns_items() {
    ...
    assert!(labels.contains(&"bar") || labels.contains(&"baz"),

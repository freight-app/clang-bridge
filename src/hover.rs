...
pub fn reparse(tu: &crate::TranslationUnit, content: Option<&str>) -> bool {
    match content {
        Some(s) => {
            let Ok(cs) = CString::new(s) else { return false; };
            unsafe { ffi::cb_transunit_reparse(tu.0, cs.as_ptr(), s.len()) == 1 }
        }
        None => unsafe { ffi::cb_transunit_reparse(tu.0, std::ptr::null(), 0) == 1 },
    }
}
